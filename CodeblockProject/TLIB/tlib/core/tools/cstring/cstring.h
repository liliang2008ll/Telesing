...
class SafeBuffer {
public:
	SafeBuffer(int nBufferLen) {
		if (nBufferLen > 0)
			m_pBuffer = new char[nBufferLen];
		else
			m_pBuffer = 0;
		m_nLength = nBufferLen;
		memset(m_pBuffer, 0, nBufferLen);
	}
	~SafeBuffer() {
		if (m_pBuffer)
			delete[] m_pBuffer;
...
inline CString Space(int nLength) {
	return CString(' ', nLength);
}
...
inline NameList::NameList(NameList& names) {
	m_strList = names.m_strList;
	m_strSeparator = names.m_strSeparator;
}

inline NameList& NameList::operator=(NameList& names) {
	m_strList = names.m_strList;
	m_strSeparator = names.m_strSeparator;
	return *this;
}

inline CString NameList::Get() {
...
inline CString NameList::ToString() {
	return m_strList;
}

inline NameList& NameList::operator=(CString& s) {
	m_strList = s;
	return *this;
}
