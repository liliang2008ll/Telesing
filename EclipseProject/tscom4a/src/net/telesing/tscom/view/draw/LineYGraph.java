...
public class LineYGraph extends View {
	...
	public LineYGraph(Context context){
		this(context,null);
	}
	public LineYGraph(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public float getMaxX(){
		if (isMaxYUserSet){
			...
			maxX = lines.get(0).getPoint(0).getX();
			for (Line line : lines){
	...
	public void onDraw(Canvas ca) {
		if (fullImage == null || shouldUpdate) {
			...
			String max = (int)maxX+"";// used to display max
			String min = (int)minX+"";// used to display min
			paint.reset();
			float bottomPadding = 0, topPadding = 0;
			float sidePadding = DisplayUtil.getPx(getContext(),160);
...
//						Log.e("======>",lastXPixels+","+lastYPixels+","+ newXPixels+","+ newYPixels);
						canvas.drawLine(lastXPixels-DisplayUtil.getPx(getContext(),87), lastYPixels, newXPixels-DisplayUtil.getPx(getContext(),87), newYPixels, paint);
